...
5. **Test execution** — runs **only** the tests impacted by the Epic/Story, always **headed** (`--headed`, never headless) — never the full suite or unrelated pre-existing specs. On a re-run, the **full impacted scope is executed again from scratch**, not just what changed since last time. Failing tests get **at most one retry**; it does not loop trying fixes indefinitely — if a failure survives one retry and one considered fix attempt, it's carried into analysis as a possible genuine issue rather than debugged forever. Manual-only scenarios are walked live via MCP (also headed by nature) where feasible and otherwise listed as requiring manual execution.
...
- **Playwright CLI** (`npx playwright ...`, via Bash) — for anything script-authoring and test-execution related: `codegen` to derive selectors when scaffolding a new UI spec, `test --list` to confirm scope, and `show-report`/`test-results` to analyze results. Test execution splits by type — UI specs run with `test --headed`/`test --grep "..." --headed`, API specs run plain/headless — see below.
...
### Headed for UI and manual, headless for API

This is split by test type, not blanket: **UI** seeding, UI test execution, and manual/MCP-driven walkthroughs all use a **visible browser window** — deliberate, so a human can watch the app being seeded and tested. **API** seeding and API test execution run **headless**, since they're HTTP-level wrapper-class calls with no page to render.

- UI runs always get an explicit `--headed` flag (never rely on the project's default); the MCP server is inherently headed for manual/live work.
- API runs (`spec/api/...`) never get `--headed` — plain headless invocations are correct there.
- A run touching both types executes them as separate CLI invocations rather than forcing everything into one headed run.
- If `playwright.config.ts` defaults a **UI** project the agent touches to headless, that's flagged as something to resolve — this doesn't apply to API projects, which are expected to stay headless.
