...
### Headed vs. headless — split by test type, not blanket

Only **UI** work and **manual/MCP-driven** scenario walkthroughs run in a **visible browser window**. **API** test execution and seeding — for either domain — has no browser to show and runs **headless**, since it's HTTP-level wrapper-class calls, not a page render. Applies to both step 3 (seeding) and step 5 (execution) in every environment (`dev`/`qa`).

- **API seed scripts (step 3) and API test execution (step 5):** run via `npx playwright test spec/api/... ` with no `--headed` flag — plain headless is correct and expected here. Do not add `--headed` to API-only runs; there's no browser involved.
- **UI seed flows (step 3) and UI test execution (step 5):** always pass `--headed` (e.g. `npx playwright test spec/ui/pim/employee-list.spec.ts --headed`). Never rely on a bare `npx playwright test` if the project config defaults to headless for UI specs.
- **Manual/MCP-driven scenario walkthroughs (step 5):** the Playwright MCP server already drives a real, visible browser by nature — no extra flag needed, but never launch it with a `--headless` argument if you're the one starting the MCP server process.
- If `playwright.config.ts` sets `use: { headless: true }` (globally or per-project) for a **UI** project this agent touches, that's a blocker to flag — surface it and either get confirmation to change the config default or pass `--headed` every time. This does not apply to API projects, which are expected to stay headless.
- When a run mixes API and UI specs, execute them as separate `npx playwright test` invocations (API without `--headed`, UI with `--headed`) rather than one combined headed run — don't force API tests into a visible browser just because UI tests in the same run need one.
...
- Seeding follows the same split as execution (see "Headed vs. headless" under Tooling): API seed scripts run headless (no browser involved), UI seed flows run headed in a visible browser.
...
- Execute via the **Playwright CLI**, respecting `ENV`, with the headed/headless split from the Tooling section: API specs run plain/headless (`npx playwright test spec/api/... `), UI specs run with the browser visible (`npx playwright test spec/ui/... --headed`, `npx playwright test --grep "<title>" --headed`). Never drop `--headed` on UI runs to "go faster," and never add it to API-only runs.
